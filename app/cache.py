...
# ── Binary blob cache (rendered audio) ──────────────────────────────────────
# WAV renders are megabytes, not KB — they get their own subdirectory and
# a much smaller LRU cap so audio can never evict OMR results.

_BLOB_DIR = "audio"
MAX_BLOB_ENTRIES = 24


def _blob_path(key: str, suffix: str) -> Path:
    d = CACHE_DIR / _BLOB_DIR
    d.mkdir(parents=True, exist_ok=True)
    return d / f"{key[:_KEY_LEN]}{suffix}"


def get_blob(key: str, suffix: str) -> bytes | None:
    p = _blob_path(key, suffix)
    if not p.exists():
        return None
    try:
        data = p.read_bytes()
...
def put_blob(key: str, suffix: str, data: bytes) -> None:
    try:
        _blob_path(key, suffix).write_bytes(data)
    except OSError as exc:
        log.warning("blob cache write failed: %s", exc)
        return
    try:
        files = sorted(
            (CACHE_DIR / _BLOB_DIR).iterdir(), key=lambda f: f.stat().st_mtime
        )
        for f in files[: max(0, len(files) - MAX_BLOB_ENTRIES)]:
